...
if($_SESSION['course_upload'] != "1" && $_SESSION['admin']!="1")
{
	...
	if(($_SERVER['REQUEST_METHOD'] == "POST") && isset($_POST['submit']))
	{
		...
		if( $level !="" || $txtdept !="" || $txtfaculty !="" || $c_title !="" || $c_code !=""){
			
			//check if the record exist before
			$sql = "SELECT student_regno FROM atbu_student where student_regno=?";
			$stmt2 = $conn->prepare($sql);
			$stmt2->execute(array($c_title));
			if ($stmt2->rowCount () >= 1)
			{
				//update the record
				$stmt = $conn->prepare("UPDATE atbu_student SET student_name = ?,student_level = ?,student_dept = ?,student_facult = ?,student_password = ? WHERE student_regno=? Limit 1");
				$stmt->execute(array($c_code,$level,$txtdept,$txtfaculty,"password",$c_title));
				$affected_rows = $stmt->rowCount();
				if($affected_rows==1){
					$err =  "<p style='color:blue'>Change Applied</p>";
					$lecture =$level =$semester =$c_unit =$c_title=$c_code=$txtfaculty =$txtdept="";
				}
				else
				{
				...
				$status ="0";
					...
					$sth = $conn->prepare ("INSERT INTO atbu_student (student_name,student_level,student_dept,student_facult,student_password,student_regno)
														VALUES (?,?,?,?,?,?)");								
					$sth->bindValue (1, $c_code); 
					$sth->bindValue (2, $level); 
					$sth->bindValue (3, $txtdept);
					$sth->bindValue (4, $txtfaculty); 
					$sth->bindValue (5, "password"); 
					$sth->bindValue (6, $c_title);
					if($sth->execute()){
		...
		<div class="row">
				...
				<div  class="col-sm-12 col-md-12 col-lg-12">
					...
					<div  class="col-sm-12 col-md-12 col-lg-12"  class="col-lg-12" style="width:100%; padding-top:10px; padding-left:15px;padding-right:15px; padding-bottom:5px; background-color:#D8D8D8 ;margin-bottom:1%">
						<h4 style="text-align:center;color:black"> Register / Upload New Student Details </h4>
						<hr>
						<form role="form"  name="reg_form"  id="form" class="form-horizontal" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data" method="POST">
								...
								<div class="form-group">
									<label for="c_title" class="control-label col-xs-3">Registration N<u>o</u> :<span style="color:red" class"require">*</span></label>
										<div class="col-xs-5">
											<div class="input-group">
												<span class="input-group-addon"><span class="glyphicon glyphicon-pencil"></span></span>
								...
								<div class="form-group">
									...
									<label for="" style="color:red" class="control-label col-xs-6"><?php echo $err;?></label>
									<div class="col-xs-4">
										<div class="input-group">
