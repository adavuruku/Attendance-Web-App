		...
		if($lecture !="" || $level !="" || $semester !="" || $c_unit !="" || $c_title !="" || $c_code !=""){
			...
			if ($stmt2->rowCount () >= 1)
			{
				...
				$status ="0";
					...
					$sth = $conn->prepare ("INSERT INTO atbu_course (Course_Code,Course_Title,School,Department,Level,Semester,course_unit,staff_id)
														VALUES (?,?,?,?,?,?,?,?)");															
					$sth->bindValue (1, $c_code); 
					$sth->bindValue (2, $c_title); 
					$sth->bindValue (3, $_SESSION['faculty']);
					$sth->bindValue (4, $_SESSION['department']); 
					$sth->bindValue (5, $level); 
					$sth->bindValue (6, $semester);
					$sth->bindValue (7, $c_unit);
					$sth->bindValue (8, $lecture);
					if($sth->execute()){
		...
		<div class="row">
				...
				<div  class="col-sm-12 col-md-12 col-lg-12">
					...
					<div  class="col-sm-12 col-md-12 col-lg-12"  class="col-lg-12" style="width:100%; padding-top:10px; padding-left:15px;padding-right:15px; padding-bottom:5px; background-color:#D8D8D8 ;margin-bottom:1%">
						<h4 style="text-align:center;color:black"> Upload Courses For - <?php echo $_SESSION['department'];?> Dapartment In Faculty <?php echo $_SESSION['faculty'];?> </h4>
						<hr>
						<form role="form"  name="reg_form"  id="form" class="form-horizontal" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data" method="POST">
								...
								<div class="form-group">
									<label for="c_code" class="control-label col-xs-3">Course Code :<span style="color:red" class"require">*</span></label>
										<div class="col-xs-5">
											<div class="input-group">
												<span class="input-group-addon"><span class="glyphicon glyphicon-pencil"></span></span>
													<input  type="text"  class="text_field form-control"  id="c_code" name="c_code" value="<?php echo $c_code; ?>" placeholder="Enter The Course Code"> </input>
											</div>
										</div>
								</div>
								<div class="form-group">
								...
								<div class="form-group">
									...
									<div class="col-xs-5">
											...
											<select class="form-control"  id="lecture" value="<?php echo $lecture; ?>" name="lecture">
													<?php
														$sql = "SELECT * FROM atbu_staff order by staff_name Asc";
														$stmt2 = $conn->prepare($sql);
														$stmt2->execute();
														if ($stmt2->rowCount () >= 1)
														{
								...
								<div class="form-group">
									...
									<label for="" class="control-label col-xs-5"><?php echo $err;?></label>
									<div class="col-xs-3">
										<div class="input-group">
