...
if ($stmt->rowCount () >= 1)
{
	...
	while($row_retrieve = $stmt->fetch(PDO::FETCH_ASSOC)) 
	{
		...
		$stmt21->execute(array($user_mail,"1"));
		...
		$html1 .= '<tr rowspan="2">
			<td style="text-align:right;"><i>Delivered By :</i></td>
			<td colspan="2"></td>
			<td style="text-align:right;"><i>Sign / Date :</i></td>
			<td></td>
		</tr><tr >
			<td colspan="5"></td>
		</tr>';
...
$stmt = $conn->prepare("UPDATE cart_stock SET status = ? WHERE status=?");
$stmt->execute(array("2","1"));
$affected_rows = $stmt->rowCount();
$pdf->Output('Goods_Delivery_Slip.pdf', 'I');
